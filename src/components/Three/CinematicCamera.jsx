...
const CinematicCamera = ({ onFinaleFinished, onShowBook, userName, onCanExit }) => {
  ...
  // Credits State
  const [creditStep, setCreditStep] = useState(0); // 0: Off, 1: Name, 2: Contact, 3: Thanks, 4: Pray
  const [showBook, setShowBook] = useState(false);
  
  const credit1Opacity = useRef(0);
  ...
  const credHeaderRef = useRef();
  const credHeaderMatRef = useRef();
  const credNameRef = useRef();
  const credNameMatRef = useRef();
  const credRoleRef = useRef();
  const credRoleMatRef = useRef();
  ...
  useEffect(() => {
    ...
    if (showFinale) {
      ...
      // Step 2: Contact (Shifted +7s)
      addTimer(() => setCreditStep(2), 16000);

      // Step 3: Thanks (Shifted +7s)
      addTimer(() => setCreditStep(3), 22000);

      // Step 4: #Lekas Pulih (Shifted +7s)
      addTimer(() => setCreditStep(4), 28000);

      // Step 5: Greeting (ShowSecondFinale) - This is the "Terakhir" (Shifted +7s)
      addTimer(() => {
      ...
      // End
      addTimer(() => {
         // setShowSecondFinale(false); // Optional: keep it or fade out
         // if (onShowBook) onShowBook(); 
      }, 47000);
  ...
  const values = useControls("Cinematic Sequence", {
    "General": folder({
      playbackMode: { options: ["Sequence", "Loop Shot", "Manual Scrub"], value: "Sequence", label: "Playback Mode" },
      selectedShot: { value: 1, min: 1, max: 5, step: 1, label: "Selected Shot #", hint: "For Loop/Manual modes" },
      scrub: { value: 0, min: 0, max: 1, label: "Scrub Progress", hint: "Only works in Manual Scrub mode" },
    ...
    "Shot 1": folder({
      ...
      s1_duration: { value: 16.2, min: 1, max: 30, step: 0.5, label: "Duration" },
    }),
    "Shot 2": folder({
      ...
      s2_duration: { value: 11.5, min: 1, max: 30, step: 0.5, label: "Duration" },
    }),
    "Shot 3": folder({
    ...
    "Shot 4": folder({
      ...
      s4_duration: { value: 6.2, min: 1, max: 30, step: 0.5, label: "Duration" },
    }),
    "Shot 5": folder({
      ...
      s5_duration: { value: 5.5, min: 1, max: 30, step: 0.5, label: "Duration" },
    }),
    "Finale Text": folder({
    ...
    "Credits": folder({
        credHeader: { value: "Created by :", label: "Header" },
        credName: { value: "Yohanes", label: "Your Name" },
        credRole: { value: "", label: "Role" },
        credContact: { value: "Contact : yohhannes", label: "Contact" },
        credThanks: { value: "Special Thanks\n@WawaSensei", label: "Thanks", rows: 2 },
    ...
    "Book": folder({
       ...
       bookRotation: { value: [0, -Math.PI / 2, 0], step: 0.1, label: "Rotation" },
       bookScale: { value: 0.5, min: 0.1, max: 2, step: 0.1, label: "Scale" },
    })
  });

  const shots = [
    { start: values.s1_start, end: values.s1_end, target: values.s1_target, duration: values.s1_duration },
    { start: values.s2_start, end: values.s2_end, target: values.s2_target, duration: values.s2_duration },
    { start: values.s3_start, end: values.s3_end, target: values.s3_target, duration: values.s3_duration },
    { start: values.s4_start, end: values.s4_end, target: values.s4_target, duration: values.s4_duration },
    { start: values.s5_start, end: values.s5_end, target: values.s5_target, duration: values.s5_duration },
  ];
  ...
  useFrame((state) => {
    ...
    if (values.playbackMode === "Sequence") {
      ...
      activeIndex = Math.max(0, Math.min(Math.floor(values.selectedShot) - 1, shots.length - 1));
    }
    ...
    const targetOneOpacity = (isFinished && !showSecondFinale && creditStep === 0 && !showBook) ? 1 : 0;
    textOpacity.current = THREE.MathUtils.lerp(textOpacity.current, targetOneOpacity, 0.02);

    // Text 2 (Greeting) shows when triggered
    const targetTwoOpacity = (showSecondFinale) ? 1 : 0;
    secondTextOpacity.current = THREE.MathUtils.lerp(secondTextOpacity.current, targetTwoOpacity, 0.02);
    
    // Credits Opacities
    credit1Opacity.current = THREE.MathUtils.lerp(credit1Opacity.current, creditStep === 1 ? 1 : 0, 0.03);
    credit2Opacity.current = THREE.MathUtils.lerp(credit2Opacity.current, creditStep === 2 ? 1 : 0, 0.03);
    credit3Opacity.current = THREE.MathUtils.lerp(credit3Opacity.current, creditStep === 3 ? 1 : 0, 0.03);
    credit4Opacity.current = THREE.MathUtils.lerp(credit4Opacity.current, creditStep === 4 ? 1 : 0, 0.03);
    ...
    if (text2MatRef.current) text2MatRef.current.opacity = secondTextOpacity.current;

    // Credits 1 Items (Header, Name, Role)
    [credHeaderRef, credNameRef, credRoleRef].forEach(ref => {
        if (ref.current) {
            ref.current.fillOpacity = credit1Opacity.current;
            ref.current.outlineOpacity = credit1Opacity.current;
        }
    });
    [credHeaderMatRef, credNameMatRef, credRoleMatRef].forEach(ref => {
        if (ref.current) ref.current.opacity = credit1Opacity.current;
    });
    ...
    if (credContactMatRef.current) credContactMatRef.current.opacity = credit2Opacity.current;
    ...
    if (credThanksMatRef.current) credThanksMatRef.current.opacity = credit3Opacity.current;
    ...
    if (credPrayMatRef.current) credPrayMatRef.current.opacity = credit4Opacity.current;
  ...
  return (
    <>
       ...
       <Billboard position={values.textPosition}>
           ...
           >
             {values.credHeader}
             <meshStandardMaterial ref={credHeaderMatRef} color={values.credColor} emissive={values.credColor} emissiveIntensity={1.5} transparent opacity={0} />
           </Text>
           <Text
             ref={credNameRef}
             position={[0, -1, 0]}
           ...
           >
             {values.credName}
             {/* ADDED Emissive to make sure it pops */}
             <meshStandardMaterial ref={credNameMatRef} color="#ffffff" emissive="#ffffff" emissiveIntensity={0.8} transparent opacity={0} />
           </Text>
           <Text
             ref={credRoleRef}
             position={[0, -2.5, 0]}
           ...
           >
             {values.credRole}
             <meshStandardMaterial ref={credRoleMatRef} color="#cccccc" transparent opacity={0} />
           </Text>
       </Billboard>
       ...
       <Billboard position={values.textPosition}>
           ...
           >
             {values.credContact}
             {/* ADDED Emissive */}
             <meshStandardMaterial ref={credContactMatRef} color="#ffffff" emissive="#ffffff" emissiveIntensity={0.8} transparent opacity={0} />
           </Text>
       </Billboard>
       ...
       <Billboard position={values.textPosition}>
           ...
           >
             {values.credThanks}
             <meshStandardMaterial ref={credThanksMatRef} color={values.credColor} emissive={values.credColor} emissiveIntensity={1} transparent opacity={0} />
           </Text>
       </Billboard>
       ...
       <Billboard position={values.textPosition}>
           ...
           >
             {values.credPray}
             <meshStandardMaterial ref={credPrayMatRef} color="#ffffff" emissive="#ffffff" emissiveIntensity={0.5} transparent opacity={0} />
           </Text>
       </Billboard>
