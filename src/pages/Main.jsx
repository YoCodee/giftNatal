      ...
      return (
        <Html center>
          <div className="bg-red-900/90 text-white p-8 rounded-xl max-w-lg border border-red-500 backdrop-blur-xl">
            ...
            <p className="mb-2">Gagal memuat aset 3D. Kemungkinan file terlalu besar atau korup.</p>
    ...
    <Html center>
      <div className="relative flex flex-col items-center justify-center p-10 rounded-3xl overflow-hidden min-w-[320px] shadow-[0_0_50px_rgba(220,38,38,0.3)]">
        ...
        <div className="relative z-10 flex flex-col items-center">
            ...
            <p className="text-xs text-yellow-500/80 mb-6 uppercase tracking-[0.2em]">Please Wait</p>
...
const Main = () => {
  ...
  React.useEffect(() => {
    const onKeyDown = (e) => {
      ...
      const isLookingAtTrigger = interactableName && interactableName.toLowerCase().includes("asiap");
      
      if (isLookingAtTrigger && !cinematicMode && (e.code === "KeyF" || e.key.toLowerCase() === "f")) {
  ...
  }, [interactableName, cinematicMode, overlayOpen, questStep, dialogContent.lockedCinematic]);
  ...
  const {
    ...
    viewerPadding: { value: 10, min: 0, max: 10, step: 0.05, folder: "3D Overlay" },
  ...
  const getObjective = (step) => {
    switch(step) {
      case 0: return "Dengarkan Santa";
      case 1: return "Cari Surat di Meja ✉️";
      case 2: return "Kembali Bicara dengan Santa";
      case 3: return "Buka Kado Biru di Meja 🎁";
      case 4: return "Selesaikan Dialog";
      case 5: return "Cari Jendela & Masuk Cinematic Mode 🎥";
      default: return "Nikmati Suasana Natal";
    }
  };
    ...
    <div className="h-screen w-full bg-black ">
        ...
        <div className="fixed bottom-6 right-6 z-30 pointer-events-none animate-in fade-in slide-in-from-right-4 duration-1000 delay-300">
           <div className="bg-black/40 backdrop-blur-sm border border-white/10 rounded-lg p-3 shadow-lg flex flex-col items-end gap-1 text-[10px] sm:text-xs font-mono text-gray-300">
              <div className="flex items-center gap-2">
                 <span className="bg-white/10 px-1.5 py-0.5 rounded text-white font-bold">ESC</span>
                 <span>Show Cursor</span>
              </div>
              <div className="flex items-center gap-2">
                 <span className="bg-white/10 px-1.5 py-0.5 rounded text-white font-bold">CLICK</span>
                 <span>Lock View</span>
              </div>
              <div className="flex items-center gap-2">
                 <span className="bg-white/10 px-1.5 py-0.5 rounded text-white font-bold">WASD</span>
                 <span>Move</span>
              </div>
           </div>
        </div>
      ...
      {!cinematicMode && !overlayOpen && !overlay3DOpen && !showDialog && (
        <div className="fixed top-6 left-6 z-30 pointer-events-none animate-in fade-in slide-in-from-left-4 duration-700">
           <div className="bg-black/40 backdrop-blur-md border border-white/10 rounded-xl p-4 shadow-xl">
              <h3 className="text-yellow-500/80 text-xs uppercase tracking-widest font-bold mb-1">Current Mission</h3>
      ...
      <DialogBox 
        ...
        onComplete={() => {
          setShowDialog(false);
          if (questStep === 0) {
            setQuestStep(1); // Ready to find message
          } else if (questStep === 2) {
             setQuestStep(3); // Ready to find gift
          } else if (questStep === 4) {
             setQuestStep(5); // Cinematic Unlocked
          }
        }} 
      />
              ...
              <Player
                ...
                onInteract={(name) => {
                  ...
                  if (name === "Object_6") {
                    ...
                    const found = interactables?.find((o) => o?.name === name);
                ...
                disabled={overlayOpen || overlay3DOpen || cinematicMode || showDialog || showBookOverlay}
              />
      ...
      {interactableName && !interactableName.toLowerCase().includes("asiap") && !cinematicMode && (
        <div
          style={{
            position: "fixed",
            bottom: 28,
        ...
        pages={[
          ...
          <div className="text-center space-y-6">
             ...
             <div className="text-xl text-gray-800 leading-relaxed font-medium">
               <p className="mb-4">Halo <span className="font-bold text-green-700 text-2xl">{userName}</span>,</p>
          ...
          <div className="space-y-6 text-lg text-gray-700 leading-relaxed">
             <div className="flex items-start gap-4">
               ...
               <p>
                 Lihatlah kembali semua yang telah Anda capai—semua tantangan yang
                 Anda taklukkan, semua rintangan yang Anda ubah menjadi batu
                 loncatan.
               </p>
             </div>
             <p className="pl-14">
               Itu semua adalah bukti dari <span className="font-bold text-red-600">kekuatan, ketahanan, dan semangat luar biasa</span> yang ada di dalam diri Anda.
             </p>
          ...
          <div className="flex flex-col h-full justify-center space-y-6 text-lg text-gray-700">
             <p>
               Semangat Natal ini adalah momentum sempurna untuk mengisi ulang energi.
               Biarkan kehangatan dan cahaya perayaan ini membangkitkan tekad Anda.
             </p>
             <p>
                Ingatlah, potensi Anda tidak terbatas. Apa yang sudah Anda lakukan di 2025 hanyalah <span className="font-bold text-red-600 text-xl border-b-2 border-red-400">Permulaan</span>.
             </p>
             <div className="mt-auto pt-8 text-center bg-gradient-to-r from-red-600 to-red-700 text-white py-6 rounded-xl shadow-lg transform rotate-1 hover:scale-105 transition-all duration-300">
                <p className="font-serif text-2xl font-bold">Selamat Natal & Tahun Baru!</p>
                <p className="text-sm text-red-100 mt-2 font-medium tracking-wider uppercase">Sambut masa depan penuh kemenangan</p>
             </div>
          </div>
        ]}
      />
